...
useEffect(() => {
  const token = (cookies.get('usertoken') == undefined) ? 'no' : cookies.get('usertoken');
    ...
    axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/users/getuser`,{token:token }).then((res) => {
      ...
      if (!res.data.user.isProfileSet && window.location.pathname != '/agent/setting') {
         window.location.href = '/agent/setting'
      }
  ...
  return (
    <div className={`${inter.className} flex min-h-screen bg-background text-primary`}>
      <div className="flex flex-grow">
        {isDashboardPage ? (
          <Sidebar className="w-64 hidden md:block" UserData={UserData} /> // Set the width of the sidebar as needed
        ) : null}
        ...
        <div className="flex-grow">
          ...
          <div className="flex flex-row  p-1">
            ...
            <main style={{width: '100%'}} className=" md:p-8">{children}</main>
          </div>
        </div>
